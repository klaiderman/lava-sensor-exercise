...
Authoritative inputs: `task/original/Lava-Sensor-Exercise.html` (primary), `task/original/Lava-Sensor-Exercise.pdf` (8 pp; same document, printed from the HTML — word-level diff shows only CSS tokens/punctuation differences), `task/original/finding.schema.json` (**NOT SUPPLIED LOCALLY at exercise start — see AM-1**).
...
| D5 | EXPLICIT | Output "has to validate against" `finding.schema.json`; "We fix the shape because a stable one is what makes a finding usable by anything other than a person reading a terminal, and because it lets us compare submissions fairly." | Validate with the REAL file in tests and on the final output. |
...
|---|---|---|
| AM-1 | `finding.schema.json` is described as supplied ("This brief, plus finding.schema.json, which your output has to validate against") but is not on this machine. | **OPEN — blocking for validation only.** Ask the user/Lava for the real file. We will NOT reconstruct it from the brief's example; until it arrives, the example shape is the interim contract and all schema-dependent items are marked PENDING-SCHEMA. |
...
| AM-5 | Representation of "explicit unknown" for typed fields (`memory_bytes: 0`? `cores: 0`? `size_bytes`?) — depends on schema types. | PENDING-SCHEMA. Preference: never emit a fake 0 without an adjacent `*_status: "unknown"` + reason; if the schema forces a number, use 0 only with the explicit unknown marker beside it. |
...
| AM-7 | "Extra fields welcome anywhere" vs schema strictness. | PENDING-SCHEMA. If `additionalProperties: false` anywhere, extras move under `evidence`. |
