...
| B1a | INTERPRETATION | Stable = identical across runs and reboots on the same machine, independent of time, network state and our own process; derived from machine-resident identity with explicit provenance (`host_id_source`). Candidate chain (settled by research F5/F6): DMI product UUID (root-only on this kernel) → `/etc/machine-id` (0444 here) → hashed stable hardware identifiers (board serial / primary disk WWN / primary NIC MAC) → explicit unknown. Confidentiality rule: emit a keyed hash of machine-id (fixed sensor-specific key, documented), never the raw value; label clone/re-image caveats in `host_id_source`. Never a random or time-based value. | Provenance field present; test: run twice, compare. |
...
| B5a | INTERPRETATION | "Block devices attached" = whole physical/virtual disks (`/sys/block/*` excluding loop/ram/zram/dm/md unless nothing else exists), `size_bytes` = `/sys/block/<d>/size × 512` (the kernel reports `size` in fixed 512-byte sectors regardless of the device's logical block size — using `logical_block_size` would be wrong on 4Kn devices; F-R4-F37), `model` from `/sys/block/<d>/device/model` or NVMe `/sys/class/nvme/*/model`; extra fields welcome: `transport`, `rotational`, `serial` (if readable), `wwn`, partitions, dm/md/LVM/multipath layering, filesystems/mounts. Storage stack depth is a project priority (interviewer context), not a brief requirement. | Unreadable model → explicit unknown for that device, device still listed. |
| B6 | EXPLICIT | Unknown must be explicit: no empty strings, no plausible guesses. | No `""` anywhere in `machine`; unknown marker + reason. Representation settled against `task/derived/finding.schema.json` (AM-1/AM-5): strings use the literal `unknown`; integers use 0 only together with the machine-level `unknowns` object. |
| B7 | EXPLICIT | "Add whatever else you think belongs." Extra fields welcome anywhere. | Additional machine fields allowed: the derived schema leaves additionalProperties open everywhere (settled, AM-7). |
...
| C4a | INTERPRETATION | "Actually in force" without root: `sshd -T` is root-only (fails before printing: "no hostkeys available"), but `sshd -G` dumps the effective configuration before host-key loading and WORKS as uid 1000 on the Lava host (R1-F47, confirmed by probe `network.sshd_g_effective`, 85 directives). Primary source: `sshd -G` (bounded exec). Fallback when `sshd` is absent or `-G` fails: parse the config chain (path from the systemd unit/process args), applying Include-at-position + first-obtained-value-wins semantics and OpenSSH version defaults. Cross-check with runtime evidence: `ss` listeners, `ssh.socket`/`ssh.service` state, config mtime vs service start time (a newer config is not in force), PAM, our own session's auth method. Where derivation is not possible we say unknown, not "default". | Evidence names the command run and its exit code, the files read, directives found, and any defaults applied. |
...
| D6 | AMBIGUITY | Severity vs status: is severity the check's impact (static) or the reported finding's urgency (varies with status)? **Confirmed (LD-2, 2026-09-08 23:50Z):** each check declares an `impact` severity; reported `severity` = impact when status is `fail` or `unknown` (an unverifiable control is an assurance gap of the same weight); `info` when status is `pass`; observational checks are always `info`. Documented in NOTES. | Consistent rule applied by the engine, not per check by hand; tested. |
...
| AM-6 | Is attempting an in-band BMC query (`ipmitool mc info` / Get Device ID) "read only"? | **Superseded (LD-3, 2026-09-08 23:50Z):** the sensor never issues IPMI commands and never opens `/dev/ipmi0`; BMC identity comes from the sysfs attributes (`/sys/devices/platform/ipmi_bmc.0/*`), which the kernel answers with a live Get Device ID — sufficient proof that the BMC responds — under an out-of-band deadline. Client tools are inventory only. Reason: KCS worst-case blocking inside an uncancellable read on a customer host (research L40, F44/F45). |
...
| actual findings.json schema requirements | D4, D5, AM-1, AM-5, AM-7 (settled against task/derived/finding.schema.json, derived from the brief) |
