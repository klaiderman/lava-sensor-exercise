...
Phase 20b: CORRECTNESS CLOSURE. Baseline: external hiring-panel adversarial review = ADVANCE 6.7/10 (architecture + primitives strong; defect class = findings claiming more than their evidence supports; hostile Docker profile absent; matrix not a gate). Driver: `reports/CLOSURE_TABLE.md` (27 rows). Fix batch 2 COMPLETE (checkpoint 15 d08e81d; rows 1–17, 22, 23 TESTED; entailment engine gate; 202 WSL tests; prefilter clean 03:38Z). Review-2 (fresh Fable, 03:55Z): DO-NOT-SHIP — 4C/4H/6M/3L, mostly exposure-semantics classes (denied candidate dropped/false pass; primary-gid readers; sudoers.d file-vs-dir; rule-text/predicate mismatch; entailment gate bypass via opt-in load-bearing; unreadable /etc/group; hung-NFS stall). → Fix batch 3 COMPLETE (checkpoint 16 ab0a959; rows 28–41 TESTED; 220 WSL tests; Docker A/B/C + WSL validator PASS; entailment audit clean; prefilter clean 04:35Z). H3 explained (commits under rebuilds + Windows chmod no-op; deterministic 3/3 now). Lab final pass done (checkpoint 17 79953ff: gate 3/3 green, Docker A/B/C assertions 0 violations). Review-3 (fresh Fable, 04:50Z, reports/REVIEW_FINDINGS_3.md): DO-NOT-SHIP — 1C/3H/5M/6L, one shared root cause: Readers() counts the owner as a member of its own primary group (0440 root:root ⇒ false FAIL), undetermined group model ⇒ fail not unknown, protectionFromDenial tests read bits not traverse bits, SYSTEM_SECRET_STORE never opts out counted denials (permanent unknown on Ubuntu); Docker A lacked sudo so the gate could not see it. → Fix batch 4 sent 04:59Z (closure rows 42–49); lab author extending profile A (sudo + populated ssl-cert group) and rerunning after batch 4. Batch 4 COMPLETE (checkpoint 18 a17501d, 06:10Z: Readers/Traversers/Accessors over one beyondOwner, Determined⇒unknown, traverse-bit protection, one recordProtection opt-out, walk fixes, reason choke point; 244 WSL tests; prefilter clean). Lab gate rerun GREEN on the batch-4 binary (checkpoint 19 405905d: WSL gate ×3, Docker A 8/2/16, B 10/2/14, C 8/1/17, 0 assertion violations, entailment audit clean; rows 34/49 CLOSED). Review-4 (06:25Z, reports/REVIEW_FINDINGS_4.md): FIX-THEN-SHIP, 0 CRITICAL / 3 HIGH / 3 MEDIUM / 4 LOW, none false on the Lava host or stock distros; rows 50–56. USER DIRECTIVE 06:27Z: finish now — no new agents/review rounds; batch 5 + lab fixture prep stopped before any edit; rows 50–56 DEFERRED. FINAL HOST RUN DONE 06:29Z on checkpoint 19: 26 checks, 77 ms, 12/10/4, hashes verified, validator PASS (see CLOSURE_TABLE final-run section). Remaining: NOTES final → transcript staging → package → clean-room → checkpoint 20 → final report (ask user for the final Claude Code Usage figure). Then: regression + Staticcheck + Hydra → Docker A/B/C with assertions → fresh Fable adversarial reviewer ("make the sensor lie") → fixes → rerun → ONE clean final Lava-host run → closure audit → hiring-panel calibration review → NOTES.md → minimal tarball. Scope discipline: no new checks/research/HTML/prompt scoring/cost accounting.
