...
Phase 20b: CORRECTNESS CLOSURE. Baseline: external hiring-panel adversarial review = ADVANCE 6.7/10 (architecture + primitives strong; defect class = findings claiming more than their evidence supports; hostile Docker profile absent; matrix not a gate). Driver: `reports/CLOSURE_TABLE.md` (27 rows). Fix batch 2 COMPLETE (checkpoint 15 d08e81d; rows 1–17, 22, 23 TESTED; entailment engine gate; 202 WSL tests; prefilter clean 03:38Z). Review-2 (fresh Fable, 03:55Z): DO-NOT-SHIP — 4C/4H/6M/3L, mostly exposure-semantics classes (denied candidate dropped/false pass; primary-gid readers; sudoers.d file-vs-dir; rule-text/predicate mismatch; entailment gate bypass via opt-in load-bearing; unreadable /etc/group; hung-NFS stall). → Fix batch 3 COMPLETE (checkpoint 16 ab0a959; rows 28–41 TESTED; 220 WSL tests; Docker A/B/C + WSL validator PASS; entailment audit clean; prefilter clean 04:35Z). H3 explained (commits under rebuilds + Windows chmod no-op; deterministic 3/3 now). Running: test-lab-author final pass (gate x3, Docker A/B/C assertions, TEST_REPORT final), code-reviewer-3 (targeted fresh re-verification + attack on the new semantics). Next: if review-3 is SHIP → ONE final host run (tooling/host_run.sh) → closure-auditor + hiring-panel-reviewer → NOTES final → package → clean-room → ask user for final usage figure. Then: regression + Staticcheck + Hydra → Docker A/B/C with assertions → fresh Fable adversarial reviewer ("make the sensor lie") → fixes → rerun → ONE clean final Lava-host run → closure audit → hiring-panel calibration review → NOTES.md → minimal tarball. Scope discipline: no new checks/research/HTML/prompt scoring/cost accounting.
