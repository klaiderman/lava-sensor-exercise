...
if grep -rIlE "sk-ant-[A-Za-z0-9_-]{8,}|BEGIN (RSA|OPENSSH|EC) PRIVATE KEY|TARGET_HOST=|SSH_KEY_PATH=" "$STAGE" | grep -v '/transcripts/' ; then echo "SECRET-SHAPED CONTENT IN STAGE - ABORT"; exit 3; fi
if grep -rIlE "sk-ant-[A-Za-z0-9_-]{8,}" "$STAGE/transcripts"; then echo "UNREDACTED KEY IN TRANSCRIPTS - ABORT"; exit 3; fi
...
wc -w "$STAGE/NOTES.md" | awk '{ if ($1 > 700) { print "NOTES.md too long: " $1 " words"; exit 1 } else print "NOTES words: " $1 }'
