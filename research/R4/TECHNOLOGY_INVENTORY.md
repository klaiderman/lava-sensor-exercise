...
| BMC USB network gadget | DETECTED, link DOWN | OK | `bmc.usb_nic_identity`, `bmc.usb_cdc_net`, `network.nic_info` | `enxbe3af2b6059f`, `manufacturer=Linux 5.4.62 with aspeed_vhub`, `product=RNDIS/Ethernet Gadget`, `idVendor=0b1f idProduct=03ee`, `driver=rndis_host`, `state=down`; reading `speed` gave `Invalid argument` (EINVAL — the documented result for a down link) |
