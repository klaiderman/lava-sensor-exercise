...
> **Final host run 2026-09-09T06:29Z (checkpoint 19, binary sha256 9298e5c9…):** 26 checks in 77 ms, **12 pass / 10 fail / 4 unknown**, validator PASS, 0 entailment violations. Deltas against the host column above, all explained: `HOST_FIREWALL_STATE` = **fail** (host fact found after this prediction: `/etc/ufw/ufw.conf` says `ENABLED=no` while the unit is active, and a global-scope listener exists); `CREDENTIAL_FILE_EXPOSURE` = **pass** (this row predates fix batch 3 / closure row 39: for exposure questions a denied read behind an ancestor that nobody beyond the owner can traverse is protection from this account, the same rule the PRIVATE_KEY row was re-confirmed under); `BMC_INBAND_INTERFACE_PRESENT` = **unknown/EACCES** (the SMBIOS type-38 entry, the ACPI IPI0001 device and both platform devices were observed, but the entry attributes `raw`/`type` are 0400 root-only and the check marks them load-bearing, so the engine downgrades the declaration to unknown — an under-claim, recorded as a known limitation, not a false verdict). The other 23 rows match.
